...
printf '%s\n' "  service DNS names that do not resolve here, and litellm-config.yaml sets"
printf '%s\n' "  default_on: true — so every request will fail until you either set"
printf '%s\n\n' "  default_on: false in litellm-config.yaml or make Presidio reachable on localhost."
